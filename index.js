...
let delуеу = document.querySelector('.delete');
...
function addForm() {
    ...
    let newNum = clone.querySelector('.number');
...
function toAccept(event) {
    ...
    let caseOfWord = 'напитков';
    if ((counter % 10 == 1) && (counter % 100 != 11)) caseOfWord = 'напиток';
    else if  ((counter % 10 >= 2) && (counter % 10 <= 4) && ((counter % 100 < 12) || (counter % 100 > 14))) caseOfWord = 'напитка';
    document.querySelector('.text').textContent = `Вы заказали ${counter} $ caseOfWord}`;
